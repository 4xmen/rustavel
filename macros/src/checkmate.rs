...
pub fn find_validating_attr(field: &Field) -> Option<Attribute> {
    field
        .attrs
        .iter()
...
pub fn parse_rules(attr: &Attribute, fields_name: &HashSet<String>) -> Result<Vec<Rule>> {
...
pub fn parse_single_rule(raw: &str, span: Span, fields_name: &HashSet<String>) -> Result<Rule> {
    if raw.contains(':') {
...
/// or an `Option` wrapping one of those types.
pub fn is_string_collection_type(ty: &Type) -> bool {
    let ty = extract_option_inner_type(ty).unwrap_or(ty);
...
/// or an `Option` wrapping one of those types.
pub fn is_datetime_types(ty: &Type) -> bool {
    let ty = extract_option_inner_type(ty).unwrap_or(ty);
